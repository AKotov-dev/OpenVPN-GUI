...
uses
  ...
  Menus, LCLTranslator, DefaultTranslator;
...
begin
  ...
  try
    ...
    Application.ProcessMessages;
    RunCommand('/usr/bin/bash',
      ['-c', '/usr/bin/bash -c "chmod 600 /etc/openvpngui/openvpngui.pass; systemctl daemon-reload; '
      + 'systemctl stop openvpngui.service; systemctl restart openvpngui.service"'],
      output);
  finally
...
begin
  ...
  begin
    ...
    Memo1.Text := 'The logfile not found: /var/log/openvpngui.log';
  LOG.Free;
end;
...
//Стоп - Параметры $1-User $2-Password $3-Флаг останова = 1
